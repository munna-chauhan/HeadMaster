...
def should_compress(filepath: Path) -> bool:
    ...
    if filepath.name.endswith(".original.md"):
        return False
    # Skip formal pipeline artifacts — these must never be compressed
    FORMAL_ARTIFACTS = {
        "PRD.md", "PRD_REVIEW.md",
        "SYSTEM_DESIGN_NOTES.md", "TDD_REVIEW.md",
        "JIRA_BREAKDOWN.md", "MIGRATION_PLAN.md",
    }
    if filepath.name in FORMAL_ARTIFACTS:
        return False
    # Skip TDD files (TDD.md, TDD_MASTER.md, TDD_{REPO}.md)
    if filepath.name.startswith("TDD") and filepath.suffix == ".md":
        return False
