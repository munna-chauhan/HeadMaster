...
Sprint 12 #49 / Sprint 13 #54: Added Maven scope checking for embedded DBs.
...
def _check_maven_dependency_scope(repo_path: Path, group_id_pattern: str) -> bool:
    """Check if Maven dependency has test scope (or no scope, which defaults to compile)."""
    ...
    try:
        ...
        for dep in root.findall(".//m:dependency", ns):
            ...
            if group is not None and re.search(group_id_pattern, group.text, re.IGNORECASE):
                # Check scope
                if scope is None:
                    # No scope element → default is compile, assume test-available
                    return True
                if scope.text in ("test", "provided"):
                    # Test scope → available in tests
                    return True
                if scope.text in ("runtime", "compile"):
                    # Runtime/compile → NOT available in test scope by default
                    return False
        return False
...
def detect_infra(repo_path: Path, build_system: str, build_content: str) -> dict:
    ...
    for name, sig in INFRA_SIGNATURES.items():
        ...
        # Special case: embedded_db with Maven → check scope
        if name == "embedded_db" and build_system == "maven":
            # Check H2, HSQLDB, etc. with scope validation
            if _check_maven_dependency_scope(repo_path, r"h2database|com\.h2database"):
                found = True
            elif _check_maven_dependency_scope(repo_path, r"hsqldb"):
                found = True
            elif _check_maven_dependency_scope(repo_path, r"embedded-database-spring-test"):
                found = True
            elif _check_maven_dependency_scope(repo_path, r"flapdoodle"):
                found = True
        else:
            # Standard pattern matching for other infra or non-Maven
